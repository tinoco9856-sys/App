...
function triggerSlide(dir) {
    ...
    if (dir === 'left' && currentIndex > 0) {
        currentIndex--;
    } else if (dir === 'right' && currentIndex < cards.length - 1) {
        currentIndex++;
    } else {
        return;
    }
...
try {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        throw new Error(
            'getUserMedia no disponible. Usa HTTPS o localhost.'
        );
    }
    ...
    if (e.name === 'NotAllowedError') {
        ...
        statusEl.textContent =
            '❌ Permiso denegado. Permite la cámara en el navegador.';
        ...
        statusEl.textContent =
            '❌ Debes usar HTTPS o Live Server.';
